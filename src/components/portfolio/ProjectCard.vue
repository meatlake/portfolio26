...
<template>
  ...
  >
    ...
    >
      <div
        class="absolute inset-0 transition-transform duration-[800ms] ease-[cubic-bezier(0.2,0.8,0.2,1)] group-hover:scale-[1.04] bg-[repeating-linear-gradient(135deg,_oklch(0.94_0.02_var(--hue,60))_0_12px,_oklch(0.92_0.025_var(--hue,60))_12px_24px)]"
      ></div>
      <div
        class="absolute right-4.5 top-4 font-serif italic text-[28px] opacity-70 text-[oklch(0.3_0.06_var(--hue,60))]"
      >{{ num }}</div>
      <div
        class="absolute left-4.5 bottom-4 font-mono text-[11px] tracking-[0.06em] uppercase px-2.5 py-[5px] rounded-md backdrop-blur-md bg-card/80 text-[oklch(0.35_0.05_var(--hue,60))]"
      >[ {{ project.label }} ]</div>
    </div>
    <div class="pt-5.5 px-6 pb-6 flex flex-col gap-3 flex-1">
      <div class="flex gap-1.5 flex-wrap">
        <span
          ...
          :key="t"
          class="font-mono text-[10.5px] tracking-[0.04em] text-ink-2 px-2 py-[3px] rounded-md bg-bg-warm border border-line"
        >{{ t }}</span>
      </div>
      <h3
      ...
      <div class="mt-auto pt-3.5 flex items-center justify-between">
        <span class="inline-flex items-center gap-2 text-sm font-medium">
          Bekijk project
          <span class="transition-transform duration-250 group-hover:translate-x-0.75 group-hover:-translate-y-0.75">↗</span>
        </span>
